...
src_install() {
	...
	if useq babylon; then
		sed -e "s/.fs2_open/.babylon2/g" code/${PN}_r > code/babylon || die "Hardcoding binary failed"
        ...
        if useq freespace2; then
		sed -e "s/.fs2_open/.freespc2/g" code/${PN}_r > code/${PN} || die "Hardcoding binary failed"
